...
(function () {
  var menulistcontainer;
  var challengeBtn = document.createElement("img");
  ...
  var addNewProblemEventHandler = function addNewProblemEventHandler() {
    chrome.runtime.sendMessage({
      action: "getCurrentTabUrl"
    }, function (response) {
      if (response && response.currentTabUrl) {
        var roomCode = prompt("Enter the room code to share this problem:");
        if (roomCode) {
          chrome.runtime.sendMessage({
            action: "shareProblem",
            roomCode: roomCode,
  ...
  var startCountdown = function startCountdown(roomCode) {
    ...
    var updateCountdown = function updateCountdown(endTime) {
      var now = new Date().getTime();
      var distance = endTime - now;
      var minutes = Math.floor(distance % (1000 * 60 * 60) / (1000 * 60));
      var seconds = Math.floor(distance % (1000 * 60) / 1000);
      countdownContainer.innerHTML = "Time left: ".concat(minutes, "m ").concat(seconds, "s");
      if (distance < 0) {
        clearInterval(countdownInterval);
        countdownContainer.innerHTML = "Time's up!";
      }
    };
    var database = (0,firebase_database__WEBPACK_IMPORTED_MODULE_0__.getDatabase)(_index_js__WEBPACK_IMPORTED_MODULE_1__.app);
    var timerRef = (0,firebase_database__WEBPACK_IMPORTED_MODULE_0__.ref)(database, 'rooms/' + roomCode + '/timer');
    var endTime = new Date().getTime() + 20 * 60 * 1000; // 20 minutes from now

    (0,firebase_database__WEBPACK_IMPORTED_MODULE_0__.set)(timerRef, {
      endTime: endTime
    }).then(function () {
    ...
    (0,firebase_database__WEBPACK_IMPORTED_MODULE_0__.onValue)(timerRef, function (snapshot) {
      var timerData = snapshot.val();
      if (timerData && timerData.endTime) {
        var _endTime = timerData.endTime;
        updateCountdown(_endTime);
        var _countdownInterval = setInterval(function () {
          return updateCountdown(_endTime);
        }, 1000);
      }
    });
  };
  var notifyParticipants = function notifyParticipants(roomCode, problemUrl) {
    var database = (0,firebase_database__WEBPACK_IMPORTED_MODULE_0__.getDatabase)(_index_js__WEBPACK_IMPORTED_MODULE_1__.app);
    var participantsRef = (0,firebase_database__WEBPACK_IMPORTED_MODULE_0__.ref)(database, 'rooms/' + roomCode + '/participants');
    (0,firebase_database__WEBPACK_IMPORTED_MODULE_0__.onValue)(participantsRef, function (snapshot) {
      snapshot.forEach(function (childSnapshot) {
        var participant = childSnapshot.val();
        // Logic to notify participants about the new problem
        console.log("Notifying participant ".concat(participant.username, " about the new problem: ").concat(problemUrl));
      });
    });
  };
})();
